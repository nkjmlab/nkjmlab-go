...
public class GoGetHandlers {
  ...
  public GoGetHandlers(Javalin app, WebsocketSessionsManager websocketManager,
      WebApplicationConfig webAppConfig, GoTables goTables, GoAuthService authService) {
    this.app = app;
  ...
  private Handler createPlayHandler() {
    return createGoHandler(ctx -> filePath -> model -> {
      ...
      session.ifPresent(opts -> {
        ...
        model.put("problemGroupsJson", goTables.problemsTable.getProblemGroupsNode());
        ctx.render(filePath, model.build());
      });
    });
  }
  ...
  private void putUserAccounts(Builder model) {
    ...
    List<GoGetHandlers.LoginJson> loginJsons = users.stream().filter(t -> t.getT1().isStudent())
  ...
  private Handler createGamesAllHandler() {
    return createGoHandler(ctx -> filePath -> model -> {
      List<GameStateViewJson> tmp =
          goTables.gameStatesTables.readTodayGameJsons().stream().map(gsj -> {
            String gameId = gsj.gameId();
            GameStateViewJson json =
                new GameStateViewJson(gsj, goTables.handsUpTable.selectByPrimaryKey(gameId),
  ...
  private Handler createGamesHandler() {
    return createGoHandler(ctx -> filePath -> model -> {
      ...
      List<GameStateViewJson> tmp =
          gids.stream().map(gid -> goTables.gameStatesTables.readLatestGameState(gid))
              .map(gsj -> new GameStateViewJson(gsj,
  ...
  private Handler createQuestionTableHandler() {
    return createGoHandler(ctx -> filePath -> model -> {
      ...
      List<GameStateViewJson> tmp = goTables.gameStatesTables.readLatestBoardsJson(gids).stream()
          .map(gsj -> new GameStateViewJson(gsj,
              goTables.handsUpTable.selectByPrimaryKey(gsj.gameId()), 0))
          .toList();
      model.put("games", tmp);
      ctx.render(filePath, model.build());
    });
